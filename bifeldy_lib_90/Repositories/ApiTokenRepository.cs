...
namespace bifeldy_lib_90.Repositories {
    ...
    public interface IApiTokenRepository {
        ...
        Task<(List<API_TOKEN_T>, decimal, decimal)> GetAll(IDatabase db, string q = null, string page = "1", string row = "10", string sort = "user_name", string order = "asc");
        ...
        Task<bool> Update(IDatabase db, API_TOKEN_T apiToken);
    ...
    public sealed class CApiTokenRepository : CRepository, IApiTokenRepository {
        ...
        public CApiTokenRepository(IApplicationService @as, IDefaultHandler defaultHandler) {
            this._as = @as;
        ...
        public async Task<bool> Create(IDatabase db, API_TOKEN_T apiToken) {
            ...
            int res = await db.ExecQueryWithResultAsync(
                @"
                    INSERT INTO api_token_t (user_name, password, app_name)
                    VALUES (:user_name, :password, :app_name)
                ",
                sqlParam
            );
        ...
        public Task<(List<API_TOKEN_T>, decimal, decimal)> GetAll(
            IDatabase db,
            string q = null,
            string page = "1",
            string row = "10",
            string sort = "user_name",
            string order = "asc"
        ) {
            string sqlQuery = @"
                SELECT *
        ...
        public async Task<bool> Update(IDatabase db, API_TOKEN_T apiToken) {
            var sqlParam = new DynamicParameters();
            sqlParam.Add("password", apiToken.PASSWORD);
            sqlParam.Add("app_name", this._as.AppName.ToUpper());
            ...
            int res = await db.ExecQueryWithResultAsync(
                @"
                    UPDATE api_token_t
                    SET password = :password, last_login = NULL, token_sekali_pakai = NULL
